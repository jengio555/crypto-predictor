...
st.session_state.total_balance = 18.97
if “risk_level” not in st.session_state:
st.session_state.risk_level = “Conservative”
if “min_allocation” not in st.session_state:
st.session_state.min_allocation = 12.0

def read_screenshot(image_bytes):
try:
...
{
“type”: “text”,
“text”: “Please read this screenshot of a crypto trading app. Extract the following information in this exact format:\nTOTAL_BALANCE: [amount]\nPOSITIONS:\n- [CRYPTO] [DIRECTION] [LEVERAGE]X [AMOUNT] [PNL%]\nOnly include what you can clearly see. Use SHORT or LONG for direction.”
}
]
}
]
...
def parse_screenshot_data(text):
balance = None
positions = []
if text:
lines = text.split(”\n”)
for line in lines:
if “TOTAL_BALANCE:” in line:
try:
balance = float(line.split(”:”)[1].strip().replace(”$”, “”).replace(”,”, “”))
except Exception:
pass
...
st.sidebar.markdown(”### Upload Invo Screenshot”)
uploaded_file = st.sidebar.file_uploader(“Upload your Invo wallet screenshot”, type=[“jpg”, “jpeg”, “png”])
...
result = read_screenshot(image_bytes)
if result:
detected_balance, detected_positions = parse_screenshot_data(result)
if detected_balance:
st.success(“Portfolio detected!”)
st.write(“Balance: $” + str(detected_balance))
if detected_positions:
st.write(“Open Positions:”)
for pos in detected_positions:
st.write(”- “ + pos)
else:
st.warning(“Could not read balance. Please enter manually.”)
...
“Total Balance ($)”,
min_value=1.0,
value=float(detected_balance) if detected_balance else st.session_state.total_balance,
step=1.0
)

risk_input = st.sidebar.selectbox(
“Risk Level”,
[“Conservative”, “Moderate”, “Aggressive”],
index=[“Conservative”, “Moderate”, “Aggressive”].index(st.session_state.risk_level)
)

min_alloc_input = st.sidebar.number_input(
“Minimum Allocation on Invo ($)”,
min_value=1.0,
value=st.session_state.min_allocation,
step=1.0,
help=“The minimum amount required to make a trade on Invo”
)

if st.sidebar.button(“Apply Settings”, type=“primary”, use_container_width=True):
st.session_state.total_balance = balance_input
st.session_state.risk_level = risk_input
st.session_state.min_allocation = min_alloc_input
st.sidebar.success(“Settings applied!”)

total_balance = st.session_state.total_balance
risk_level = st.session_state.risk_level
min_allocation = st.session_state.min_allocation
...
if detected_positions:
st.sidebar.markdown(”—”)
st.sidebar.markdown(”### Current Open Positions”)
for pos in detected_positions:
st.sidebar.info(pos)
...
if top_3:
st.markdown(”#### Best Trades You Can Afford Right Now:”)
...
st.warning(“Your balance of $” + str(total_balance) + “ is below the minimum allocation of $” + str(min_allocation) + “. Consider depositing more funds.”)
...
if can_afford:
st.write(“Suggested: $” + str(suggested_amount) + “ at “ + str(suggested_leverage) + “X leverage”)
else:
st.error(“Insufficient funds - need $” + str(suggested_amount) + “ minimum”)
...
if close_alert:
st.warning(“CLOSE POSITION ALERT!”)
...
st.sidebar.error(“Close “ + alert + “ now!”)
