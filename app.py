...
def get_signal(crypto):
    ...
    if abs(prediction) > 0.02:
        ...
        multiplier = "1X"
    return signal, multiplier, current_price, take_profit, stop_loss, chart_data
...
for left, right in pairs:
    ...
    for col, crypto in zip([col1, col2], [left, right]):
        with col:
            st.markdown("### " + crypto)
            try:
                signal, multiplier, current_price, take_profit, stop_loss, chart_data = get_signal(crypto)
                st.write("Signal: " + signal + " | Multiply: " + multiplier)
