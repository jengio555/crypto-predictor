...
def get_signal(crypto):
    ...
    train_data = ticker.history(period="90d", interval="1h")
    ...
    feature_cols = ["ma_5", "ma_20", "ma_50", "rsi", "macd", "macd_signal", "bollinger_upper", "bollinger_lower", "bollinger_width", "Volume", "volume_ma", "volume_ratio", "price_momentum", "volatility", "high_low_range", "close_position"]
    ...
    return signal, suggested_amount, suggested_leverage, current_price, take_profit, stop_loss, rsi_val, rsi_note, accuracy, confidence_label, sentiment_label, fg_label, close_alert, combined_score, can_afford
    coin_name = crypto_names[crypto]
    ticker = yf.Ticker(crypto)
...
return signal, suggested_amount, suggested_leverage, current_price, take_profit, stop_loss, rsi_val, rsi_note, accuracy, confidence_label, sentiment_label, fg_label, close_alert, can_afford, combined_score
gb = GradientBoostingRegressor(n_estimators=100, max_depth=5, learning_rate=0.05, random_state=42)
ensemble = VotingRegressor([("rf", rf), ("gb", gb)])
ensemble.fit(X_train, y_train)
accuracy = ensemble.score(X_test, y_test)
...
rsi_val = float(train_data["rsi"].iloc[-1])
close_alert = rsi_val > 70 or rsi_val < 30

save_prediction(crypto, signal, prediction, current_price)

return signal, current_price, take_profit, stop_loss, rsi_val, accuracy, confidence, close_alert, combined_score
gb = GradientBoostingRegressor(n_estimators=100, max_depth=5, learning_rate=0.05, random_state=42)
ensemble = VotingRegressor([("rf", rf), ("gb", gb)])
ensemble.fit(X_train, y_train)
accuracy = ensemble.score(X_test, y_test)
...
return signal, take_profit, stop_loss, current_price, accuracy, close_alert, combined_score
...
all_signals.sort(key=lambda x: abs(x[1][6]), reverse=True)

st.markdown("---")
st.subheader("Best Trades Right Now")

affordable = [(c, r) for c, r in all_signals if True]
top_3 = affordable[:3]

if top_3:
    for crypto, result in top_3:
        signal, take_profit, stop_loss, current_price, accuracy, close_alert, combined_score = result
        color = "green" if signal == "LONG" else "red"
        status = "CLOSE NOW!" if close_alert else "HOLD"
        st.markdown("**" + crypto + "** - :" + color + "[" + signal + "] | $" + str(round(current_price, 2)) + " | " + status)
...
for left, right in pairs:
    ...
    for col, crypto in zip([col1, col2], [left, right]):
        with col:
            st.markdown("### " + crypto)
            try:
                signal, take_profit, stop_loss, current_price, accuracy, close_alert, combined_score = next(r for c, r in all_signals if c == crypto)
                color = "green" if signal == "LONG" else "red"
                st.markdown("**Signal:** :" + color + "[" + signal + "]")
